...
namespace Bedlam
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            SudokuDLX dlx = new SudokuDLX(sod);
            int numsolns = dlx.Solve();
            ArrayList foundsolution = dlx.getSolution();
            Console.WriteLine("");
            {
                for (int s = 1; s <= dlx.GetRowsInSolution(); s++)
                {
                    Console.Write(dlx.GetSolutionRow(s) + " ");
                    if ((s % 9) == 0) Console.WriteLine("");
                }
                Console.WriteLine("");
            }
            int[,] answer = new int[9,9];
            int rowsinsol = dlx.GetRowsInSolution();
            for (int i = 1; i <= rowsinsol; i++)
            {
                answer[dlx.GetSolnCol(i), dlx.GetSolnRow(i)] = dlx.GetSolnDigit(i) + 1;
            }
            Console.WriteLine(" ");
            Console.WriteLine(" ");
            for (int r = 0; r < 9; ++r)
            ...
            {
                SortedList<ulong, Piece> thisPieceList = new SortedList<ulong, Piece>();
                AllMoves[piece_num] = thisPieceList;
                    ...
                    for (int x = 0; x < (4 - pnew.maxX); x++)
                        for (int y = 0; y < (4 - pnew.maxY); y++)
                            for (int z = 0; z < (4 - pnew.maxZ); z++)
                            {
                                ...
                                if (!thisPieceList.ContainsKey(translated.BinaryValue()))
                                    thisPieceList.Add(translated.BinaryValue(), translated);
                            }
                }
                piece_num++;
            }
